...
func TestClients(t *testing.T) {
	...
	t.Run("add_success", func(t *testing.T) {
		...
		assert.Equal(t, clients.exists(cliNoneIP), ClientSourceNone)
		assert.Equal(t, clients.exists(cli1IP), ClientSourcePersistent)
		assert.Equal(t, clients.exists(cli2IP), ClientSourcePersistent)
	})
	...
	t.Run("update_success", func(t *testing.T) {
		...
		assert.Equal(t, clients.exists(cliOldIP), ClientSourceNone)
		assert.Equal(t, clients.exists(cliNewIP), ClientSourcePersistent)
	...
	t.Run("del_success", func(t *testing.T) {
		...
		assert.Equal(t, clients.exists(netip.MustParseAddr("1.1.1.2")), ClientSourceNone)
	})
	...
	t.Run("addhost_success", func(t *testing.T) {
		...
		assert.Equal(t, clients.exists(ip), ClientSourceHostsFile)
	})
	...
	t.Run("dhcp_replaces_arp", func(t *testing.T) {
		...
		assert.True(t, ok)
		assert.Equal(t, clients.exists(ip), ClientSourceARP)

		ok = clients.AddHost(ip, "from_dhcp", ClientSourceDHCP)
		assert.True(t, ok)
		assert.Equal(t, clients.exists(ip), ClientSourceDHCP)
	})
